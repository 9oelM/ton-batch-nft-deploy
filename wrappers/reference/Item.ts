...
export class NftItem {
    ...
    static async getAddressByIndex(collectionAddress: Address, itemIndex: number): Promise<Address> {
        try {
            ...
            const response = await client.runMethod(collectionAddress, "get_nft_address_by_index", [{ type: "int", value: BigInt(itemIndex) }])
            return response.stack.readAddress()
        } catch (error) {
            console.error("getAddressByIndex error", error)
            throw error
        }
    }
}
